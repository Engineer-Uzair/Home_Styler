fetch('json/data.json') // Start fetching the data from 'data.json'
	...
	.then(result => { // Once the JSON is received
		const cardsContainer = document.getElementById("cards"); // Get the element to insert the btab cards into
		localStorage.setItem("CATGnAME", "btab")
		for (let btab of result.btab) { // Loop through each btab in the array
		   ...
		   <div class="col-md-4">
			<div class="shop_1r1l border_1 m-3">
				...
				<div class="shop_1r1l1i">
				  <div class="grid clearfix">
					 <figure class="effect-jazz mb-0">
					   <a"><img src="${btab.images}" class="w-100" alt="abc"></a>
					 </figure>
				 </div>
				</div>
			  ...
			  <div class="shop_1r1l2 text-center p-4">
			   ...
			   <h5 class="fw-bold mt-2">${btab.title}</h5>
			   <h4 class="col_yell">${btab.price}</h4>
			   <h6 class="mb-0 mt-3"><a class="button_1" href="detail.html?${btab.id}">READ MORE <i class="fa fa-long-arrow-right ms-1"></i></a></h6>
			  </div>
			</div>
		   </div>
        `;
	...
	.catch(error => console.error('Error fetching the btab data:', error)); // Log any errors that occur during fetch
